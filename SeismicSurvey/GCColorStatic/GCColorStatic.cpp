...
BEGIN_MESSAGE_MAP(CGCColorStatic, CStatic)
	//ON_MESSAGE( WM_GECCOLORSTATIC_DRAWFRAME, (void)OnDrawFrame )
	ON_MESSAGE(WM_GECCOLORSTATIC_DRAWFRAME, &CGCColorStatic::OnDrawFrame)
...
//240226 sanghkim C2440 error
//void CGCColorStatic::OnDrawFrame(WPARAM wp, LPARAM lp)
LRESULT CGCColorStatic::OnDrawFrame(WPARAM wp, LPARAM lp)
{
	if (m_bColorFrameFlag)
	{
		// Note:  I do not inflate the rectangle at which the color frame is drawn because
		...
		{
			CPen pen;
			pen.CreatePen(PS_INSIDEFRAME, m_ColorFrameWidth, m_ColorFrameColor);
			ScreenToClient(&rect);
			CClientDC dc(this);
			dc.SelectObject(pen);
		...
		catch (CException* ex)
		{
			...
			CString strMsg;
			strMsg.Format("CPen Error code %s", strMsg);
			::AfxMessageBox(strMsg);
		}
	}
	return 0; //240226 sanghkim C2440 error
}
