...
/**/ #define AFX_WNDCLASS(s) _T("Afx") _T(s) _T(_MFC_FILENAME_VER) _STATIC_SUFFIX _UNICODE_SUFFIX _DEBUG_SUFFIX
/**/ #else
...
void CBiSplitterWnd::SetPaneEmpty(CBiPane *pPane)
{
...
void CBiSplitterWnd::RecalCBiPane(UINT nPane, int left, int top, int right, int bottom)
{
	ASSERT(nPane >=0 && nPane <=1);

	CBiPane *pPane = &(m_panes[nPane]);
...
void CBiSplitterWnd::DrawPane(CDC *pDC, CBiPane *pPane)
{
	ASSERT_VALID(pDC);
...
void CBiSplitterWnd::DrawCaption(CDC *pDC, CBiPane *pPane)
{
	...
	case THIN_CAPTIONBORDER: //one pixel width pane border
		{
			if(pPane->m_rctCaption.Width() >= 2 * BorderWidth(pPane->m_bspStyles))
			{
				...
				{
					CPen pen;
					pen.CreatePen(PS_SOLID, 0, m_clrFlatBorder);
...
void CBiSplitterWnd::DrawCloseBtn(CDC *pDC, CBiPane *pPane)
{
...
//	m_panes[0].HidePane();
	RecalCBiPane(1, rctClient.left, rctClient.top, rctClient.right, rctClient.bottom);
	Invalidate(FALSE);
}
...
void CBiSplitterWnd::RclExpandFirstPane(const CRect &rctClient, BOOL isGapVisible)
{
	m_rctGap.ShowGap(isGapVisible);
//	m_panes[1].HidePane();
	RecalCBiPane(0, rctClient.left, rctClient.top, rctClient.right, rctClient.bottom);
	Invalidate(FALSE);
}
...
void CBiSplitterWnd::RclPanesV(const CRect &rctClient)
{
	...
	RecalCBiPane(0, rctClient.left, rctClient.top, m_nSplitterPos, rctClient.bottom);
	RecalCBiPane(1, m_nSplitterPos + m_nGap, rctClient.top, rctClient.right, rctClient.bottom);
...
void CBiSplitterWnd::RclPanesH(const CRect &rctClient)
{
	...
	RecalCBiPane(0, rctClient.left, rctClient.top, rctClient.right, m_nSplitterPos);
	RecalCBiPane(1, rctClient.left, m_nSplitterPos + m_nGap, rctClient.right, rctClient.bottom);
...
//draw all items
	...
	for( CBiPane *pPane = m_panes; pPane < m_panes + 2; pPane++)
	{	
		DrawPane(&dc, pPane);
	}
