...
{	
    ...
    GetParent()->SetWindowText(GetDocument()->GetTitle()+"-3D Contour");
	...
	if(nInterpolater == 0)
		...
		pInterpolater = new Kriging(Pbegin, Pend, 4, 0);

	vector<double> vecZs;

	for(int j=0; j<=nDiameter; j++) {
		for(int i=0; i<=nDiameter; i++) {
			double z = pInterpolater->GetInterpolatedZ(j - nDiameter/2, i - 0.5 * nDiameter, Pbegin, Pend);
			vecZs.push_back(z);
		}
	}
