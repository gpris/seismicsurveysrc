...
class CBiPane //consists of border, possible caption and view window inside
{
public:
	CBiPane(): m_bspStyles(0),
		m_isGripped(FALSE),
        m_isVisible(TRUE),
        m_pView(NULL),
        m_sCaptionText(_T("")),
...
//	BOOL m_bTracking;
	...
	void RecalCBiPane(UINT nPane, int left, int top, int right, int bottom);
	void RecalcGap(int left, int top, int right, int bottom);
	void SetPaneEmpty(CBiPane *pPane);
	void DrawPane(CDC *pDC, CBiPane *pPane);
	void DrawCaption(CDC *pDC, CBiPane *pPane);
	void DrawCloseBtn(CDC *pDC, CBiPane *pPane);
