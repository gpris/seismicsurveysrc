...
void FFT(double *InputR, double *InputI, int N, TTransFormType Type)
{
 ...
 {
  TRACE("Invalid FFT type or size.");
  return;
 }
 ...
 if(BufferR == NULL  || BufferI == NULL  ||
  ...
  {
   TRACE("FFT Memory Allocation Error");
   return;
  }
...
void DFT(double *InputR, double *InputI, int N, int Type)
{
 ...
 if(SumR == NULL || SumI == NULL ||
  ...
  {
   TRACE("Incorrect DFT Type or unable to allocate memory");
   return;
  }
...
void RealSigDFT(double *Samples, double *OutputR, double *OutputI, int N)
{
 ...
 if(TwiddleReal == NULL || TwiddleImag == NULL)
  {
   TRACE("Failed to allocate memory in FIRFreqResponse");
   return;
  }
...
void WindowData(double *Data, int N, TWindowType WindowType, double Alpha, double Beta, bool UnityGain)
{
 ...
 if(WinCoeff == NULL)
  {
   TRACE("Failed to allocate memory in WindowData() ");
   return;
  }
 ...
 // It also gives the same BW as the Gauss window used in the HP 89410A Vector Signal Analyzer.
  ...
  {
   TRACE("Incorrect window type in WindowFFTData");
   delete[] WinCoeff;
   return;
  }
