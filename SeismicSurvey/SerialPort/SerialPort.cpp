						   ...
						   UINT  writebuffersize)	// size to the writebuffer
{
	...
	SetupComm(m_hComm, 20000, writebuffersize);
...
//				port->SetRTSEnable(FALSE);
				WriteChar(port);
//				Sleep(10);
//				port->SetRTSEnable(TRUE);
				break;
			}
...
//		port->m_dcb.fRtsControl = RTS_CONTROL_DISABLE;		// set RTS bit high!
//		port->SetSetCommState(m_hComm, &m_dcb);
//		port->SetRTSEnable(TRUE);
//		Sleep(10);
...
//		port->m_dcb.fRtsControl = RTS_CONTROL_ENABLE;		// set RTS bit high!
...
*/
		...
		{
			RXBuff = port->m_szReadBuffer[iii];
		    ::SendMessage((port->m_pOwner)->m_hWnd, WM_COMM_RXCHAR, (WPARAM) RXBuff, (LPARAM) port->m_nPortNr);
		}
...
int CSerialPort::DirectReadFromPort(char * string)
{
...
int CSerialPort::DirectWriteToPort(char* string)
{
	...
	char WriteBuffer[20000];
	memset(WriteBuffer, 0, sizeof(WriteBuffer));
	strcpy(WriteBuffer, string);

//	ResetEvent(m_hWriteEvent);

	int aa = strlen((char*)WriteBuffer);
//	EnterCriticalSection(&m_csCommunicationSync);
	bResult = WriteFile(m_hComm,							// Handle to COMM Port
						WriteBuffer,					// Pointer to message buffer in calling finction
						strlen((char*)WriteBuffer),	// Length of message to send
						&BytesSent,								// Where to store the number of bytes sent
...
BOOL CSerialPort::SetRTSEnable(BOOL value)
{
	if(value)
	{
		m_dcb.fRtsControl = RTS_CONTROL_ENABLE;		// set RTS bit high!
